...
class Validator {
	...
	compileRule(rule, context, path, innerSrc, resVar) {
		...
		if (item) {
			...
			if (this.opts.haltOnFirstError === true && !(rule.schema.properties || rule.schema.props)) {
				sourceCode.push("if (errors.length) return parentObj;"); // è dopo
			}
	...
	makeError({ type, field, expected, actual, messages }) {
		...
		return `errors.push({ ${s} });` + (this.opts.haltOnFirstError === true ? `return ${o.actual};` : "");
	}
